...
class MyPytorchModel(pl.LightningModule):
    ...
    def __init__(self, hparams, input_size=3 * 32 * 32, num_classes=10):
        ...
        # self.model = torch.nn.Linear(F.relu(torch.nn.Linear(self.hparams)))
        self.model = nn.Sequential(
            nn.Linear(input_size, self.hparams["hidden_size"]),
            nn.ReLU(),
            nn.Linear(self.hparams["hidden_size"], num_classes),
        )
    ...
    def prepare_data(self):
        ...
        my_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])
