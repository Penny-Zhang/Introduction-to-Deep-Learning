...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        ########################################################################
        out_width = self.max - self.min
        in_width = self._data_max - self._data_min
        images = out_width * (images-self._data_min) / in_width + self.min
...
def compute_image_mean_and_std(images):
    ...
    C = np.size(images,axis=3)
    mean = np.zeros(C)
    std = np.zeros(C)
    for d in range(C) :
    	mean[d] = np.mean(images[:,:,:,d])
    	std[d] = np.std(images[:,:,:,d])
    ...
    def __call__(self, images):
        ...
        if dimension==1 :
            ...
            for d in range(dimension):
                images[:,:,d] = (images[:,:,d]-self.mean[d]) / self.std[d]
